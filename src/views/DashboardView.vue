<template>
  <div class="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10">
    <!-- Tokens Card -->
    <div class="md:col-span-5 bg-white rounded-[2.5rem] lg:rounded-[3rem] p-10 md:p-12 shadow-xl shadow-zinc-200/40 border border-zinc-100 relative overflow-hidden group min-h-[420px] transition-all duration-500 hover:shadow-2xl hover:shadow-zinc-300/50 hover:-translate-y-2">
      <div class="relative z-10 flex flex-col h-full">
        ...
        <div class="mt-20">
          <div class="flex items-baseline mb-4">
            <span class="text-7xl font-serif tracking-tighter text-zinc-900 leading-none">{{ overview?.tokenCount || 0 }}</span>
            <span class="text-lg text-zinc-400 ml-3 font-light">总计</span>
          </div>
    ...
    <div class="md:col-span-7 flex flex-col gap-8 md:gap-10">
      <!-- Shortlinks -->
      <div class="bg-zinc-900 rounded-[2.5rem] p-10 md:p-12 shadow-xl relative overflow-hidden group flex-1 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 flex flex-col justify-center">
        <div class="absolute top-1/2 -translate-y-1/2 right-12 opacity-20 group-hover:opacity-40 transition-opacity pointer-events-none">
        ...
        <div class="relative z-10">
          ...
          <p class="text-white text-2xl font-serif mb-12">高效、极简的短链分发系统。</p>
          <div class="flex items-baseline">
            <span class="text-6xl font-serif text-white tracking-tighter leading-none">{{ overview?.linkCount || 0 }}</span>
            <span class="text-sm text-zinc-400 ml-3">路由规则</span>
          </div>
        </div>
      </div>
