  ...
  <div class="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10">
    ...
    <div class="md:col-span-5 bg-white rounded-[2.5rem] lg:rounded-[3rem] p-10 md:p-12 shadow-xl shadow-zinc-200/40 border border-zinc-100 flex flex-col justify-between relative overflow-hidden group min-h-[420px] transition-all duration-500 hover:shadow-2xl hover:shadow-zinc-300/50 hover:-translate-y-2">
      <div class="relative z-10">
        ...
        <div class="mb-4">
          <span class="text-6xl md:text-7xl font-serif tracking-tighter text-zinc-900">{{ overview?.tokenCount || 0 }}</span>
          <span class="text-lg text-zinc-400 ml-2 font-light">总计</span>
        </div>
        <p class="text-zinc-500 text-sm leading-relaxed max-w-[200px]">有效且未过期的 API 访问凭证，保障各服务的安全接入。</p>
      </div>
    ...
    <div class="md:col-span-7 flex flex-col gap-8 md:gap-10">
      ...
      <div class="bg-zinc-900 rounded-[2.5rem] p-10 md:p-12 shadow-xl flex flex-col justify-between relative overflow-hidden group flex-1 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2">
        ...
        <div class="relative z-10 mb-8">
          <h3 class="text-zinc-400 text-sm font-bold uppercase tracking-[0.2em] mb-2">Shortlinks Routing</h3>
          <p class="text-white text-2xl font-serif">高效、极简的短链分发系统。</p>
        </div>
        <div class="relative z-10 flex items-end gap-6">
          <div>
            <span class="text-5xl font-serif text-white tracking-tighter">{{ overview?.linkCount || 0 }}</span>
            <span class="text-sm text-zinc-400 ml-2">路由规则</span>
          </div>
        </div>
      </div>
